-- Scope lead dedupe to assigned user and enforce per-user uniqueness.

drop index if exists public.ux_leads_email_normalized;

create unique index if not exists ux_leads_assigned_user_email_normalized
  on public.leads (assigned_user_id, email_normalized);
...
begin
  ...
  duplicates as (
    ...
    from candidates c
    join public.leads l
      on l.assigned_user_id = p_assigned_user_id
     and l.email_normalized = c.email
  )
  select
...
begin
  ...
  where s.is_valid
    ...
    and exists (
      select 1
      from public.leads l
      where l.assigned_user_id = p_assigned_user_id
        and l.email_normalized = s.email_normalized
    );
  ...
  on conflict (assigned_user_id, email_normalized) do nothing;
